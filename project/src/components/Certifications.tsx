...
const certifications: Certification[] = [
  ...
  {
    id: 7,
    title: "IEEE Certification for Research Publication",
    issuer: "IEEE",
    date: "April 2024"
  },
  {
    ...
    issuer: "WeHack (National Level Hackathon)",
    date: "February 2024",
  },
  {
    id: 9,
    title: "Digital Twin Workshop (Participation & 3rd Place)",
    issuer: "Praya Labs, SRM Institute of Science and Technology",
    date: "October 2024"
  },
  {
    id: 10,
...
const Certifications: React.FC = () => {
  ...
  // Pagination logic: show 6 certifications per page
  const itemsPerPage = 6;
  const [currentPage, setCurrentPage] = useState(0);
  const totalPages = Math.ceil(certifications.length / itemsPerPage);
  const pages: Certification[][] = [];
  for (let i = 0; i < totalPages; i++) {
    pages.push(certifications.slice(i * itemsPerPage, i * itemsPerPage + itemsPerPage));
  }
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mx-auto">
          {/* Sliding pages of certifications */}
          <div className="overflow-hidden">
            <div className="flex transition-transform duration-500" style={{ transform: `translateX(-${currentPage * 100}%)` }}>
              {pages.map((page, pageIndex) => (
                <div key={pageIndex} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 flex-shrink-0 w-full">
                  {page.map(cert => (
                    <div 
                      key={cert.id}
                      className="bg-slate-800 rounded-lg p-6 shadow-md hover:shadow-lg transition-shadow duration-300 border border-slate-700 flex flex-col"
                    >
                      <div className="flex items-start mb-4">
                        <div className="bg-blue-900 p-2 rounded-md mr-4">
